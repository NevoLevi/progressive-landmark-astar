...
We study a fixed schedule for optimal single-agent \Astar{}: Manhattan distance
at generation, a four-landmark prefix at the first competitive pop, and all 32
landmarks at the second.  The central question is whether the prefix avoids
enough suffix evaluations to repay the extra OPEN cycle, and how that balance
varies across map topologies.

The protocol compares four methods in one deterministic graph-search engine:
Manhattan distance,
...
landmarks.  The prefix and full counts were set to $K=4$ and $M=32$ before the
formal runs.  Twelve checksum-pinned Moving AI maps define a map-disjoint
split: four development maps validate the implementation and analysis, and
eight previously untouched maps supply the reported 800-query evaluation.
Timing uses rotated blocks in which every method occurs twice in every order
position.
...
The paper contributes (i) a specified three-level evaluation schedule for a
nested differential heuristic; (ii) a correctness argument and a
trace-invariance test that separates scheduling from search order; and (iii)
a reproducible, map-disjoint decomposition of saved pivot work, extra OPEN
work, runtime, and preprocessing amortization.  Landmark lower bounds,
differential heuristics, heuristic subsets, Lazy \Astar{}, and multi-stage
evaluation are established ideas.  We found no prior node-level experiment
with this fixed-prefix schedule, although differently named work may exist.
