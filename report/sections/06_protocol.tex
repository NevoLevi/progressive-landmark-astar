...
\subsection{Prospective split and query selection}
...
summarizes the immutable plan.  The same four topology families occur on both
sides, but no map file overlaps.  Development uses one train map per family
and 40 queries per map.  It validates correctness, completeness, binding, and
the frozen analysis code; it performs no selection.  Only after an external
audit issues a freeze does sealed evaluation use two new maps per family and
100 queries per map.

\begin{table}[t]
\centering
\small
\caption{Frozen protocol.  Each query-method pair has one warm-up and eight
timed searches.}
\label{tab:design}
\begin{tabular}{lrrr}
\toprule
...
and the first 25 for evaluation.  Duplicate $(\text{map},s,t)$ keys fail
closed.  No query is sampled, replaced, or removed after timing.
...
\subsection{Gating and analysis}
...
full-landmark expansion digests must agree.  Only then is timing unlocked.
There are no post-hoc exclusions.  Mechanism measures are exact per-query
