...
preprocessing are established design dimensions.  Selecting a fixed subset
from many admissible heuristics has also been studied directly
\citep{rayner2013subset}.  Our intermediate stage uses the first four members
of a deterministic farthest-first order, without optimizing a subset on the
reported maps.
...
bound before expansion.  The Lazy \Astar{} literature notes that the same
procedure extends to more than two heuristic stages.  We evaluate the
resulting cost tradeoff for nested landmark prefixes rather than proposing
multi-stage evaluation itself.
...
Prior work establishes the individual ingredients: landmark preprocessing,
differential maxima, subset selection, lazy evaluation, and multi-stage
extension.  Here a fixed four-pivot prefix precedes a fixed 32-pivot final
bound.  The experiments measure whether suffix calls avoided on surplus nodes
outweigh the added requeues and pops.
